...
class GetProviderAssignedPetsUsecase
    implements UsecaseWithoutParams<List<PetEntity>> {
  final IPetRepository _repository;

  GetProviderAssignedPetsUsecase({required IPetRepository repository})
    : _repository = repository;

  @override
  Future<Either<Failure, List<PetEntity>>> call() async {
    try {
      final pets = await _repository.getProviderAssignedPets();
      return Right(pets);
    } catch (e) {
